...
files.keys().forEach((key) => {
    ...
    const m = files(key);
    const {store,...actionTypes} = m;
    if(!store) return false;
    let {namespace,state,reducers,actions} = store;
    if(!state || !reducers || !actions) return false;
    globalActions[namespace] = actions;
    if(actionTypes){
        globalActionTypes[namespace] = actionTypes;
    }
    if(typeof namespace!=="string" || !namespace){
        namespace = key.replace(/(\.\/|\/index\.js)/g,"");
    }
    globalReducers[namespace] = createReducer(state,reducers)
})
...
export default {
    actionTypes : globalActionTypes,
    actions : globalActions,
    reducers : combineReducers(globalReducers)
}
